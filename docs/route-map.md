...
| MG-10     | 停班/临时闭店创建 | Web 后台   | `/manager/schedule/capacity-changes/new`             | `apps/admin/src/pages/manager/capacity-change-page.tsx`            | 已完成 | 工作台与排班入口、直达恢复、影响预览和角色边界测试        |
| MG-11     | 受影响预约处理    | Web 后台   | `/manager/schedule/capacity-changes/:kind/:changeId` | `apps/admin/src/pages/manager/capacity-change-resolution-page.tsx` | 已完成 | 工作台风险入口、直接访问、刷新进度恢复与完整逐笔流程测试  |
